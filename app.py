...
if page_selection == "📊 Dashboard":
    ...
    if sla_filter != "All":
        data["SLA %"] = ((data["SLA TTO Done"] + data["SLA TTR Done"]) / 2)
        if sla_filter == "High (>=90%)":
            data = data[data["SLA %"] >= 90]
        elif sla_filter == "Medium (75–90%)":
            data = data[(data["SLA %"] >= 75) & (data["SLA %"] < 90)]
        else:
            data = data[data["SLA %"] < 75]
    ...
    if "Technician Name" in data.columns:
        ...
        st.plotly_chart(px.bar(top_techs, x="Technician Name", y="SLA %", text="SLA %"), use_container_width=True)
    ...
    if selected_techs:
        comp_data = data[data["Technician Name"].isin(selected_techs)]
